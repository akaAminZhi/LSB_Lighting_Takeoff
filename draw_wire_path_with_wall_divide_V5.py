...
def build_disjoint_families(
    ...
    *,
    ...
    """
    改进版：
      - priority_families 作为“锁定家族”（locked families）。
      - 锁定家族的标签只允许与其“允许集”内的标签合并；否则拒绝合并。
      - 仍然遵守 |family| <= max_family_size。
    """
    keep_labels = set(keep_labels)

    # 1) 收集可用点与标签；基于邻近统计共现权重
    pts, labs = [], []
    for d in dets:
    ...
    # 2) 并查集 + 每个根的“当前标签集”和“锁定允许集”
    parent = list(range(len(labels)))
    rank = [0] * len(labels)
    comp_labels: List[Set[str]] = [{lab} for lab in labels]  # 根的实际成员标签集合
    # root -> allowed set (None 表示未锁定；否则为一个不可扩张的允许集合)
    comp_allowed: List[Optional[Set[str]]] = [None] * len(labels)

    # 准备：把 priority_families 映射为“锁定允许集”，并预合并同一锁定家族内部标签
    if priority_families:
        ...
        def can_union_roots(rx: int, ry: int) -> bool:
            ...
            if B is not None and not U.issubset(B):
                return False
            # 若两个根分别锁定，且允许集不一致（互不相等），但 U 恰好都在两个允许集中，也视作允许——
            # 这种情况仅当两允许集相同或一个是另一个子集且 U 落共同子集中；若你更想“严格相同才允许”，
            # 则改成：if A is not None and B is not None and A != B: return False
            return True

        def union_roots(rx: int, ry: int):
            # 合并根（假设已经通过 can_union_roots）
            if rank[rx] < rank[ry]:
                rx, ry = ry, rx
            parent[ry] = rx
            if rank[rx] == rank[ry]:
        ...
        # 初始化：把所有锁定标签标注“允许集合”
        for allowed_set in locked_families:
            # 给该家族中的每个单标签根都标上 allowed_set
            for lab in allowed_set:
                r = idx_of_label[lab]
                comp_allowed[r] = set(allowed_set)

        # 再把同一锁定家族内部先合并（严格在 allowed_set 内不会越界）
        for allowed_set in locked_families:
            labs_sorted = sorted(allowed_set)
            base = idx_of_label[labs_sorted[0]]
            for lab in labs_sorted[1:]:
                rx, ry = find(base), find(idx_of_label[lab])
                if rx == ry:
                    continue
                if can_union_roots(rx, ry):
                    base = union_roots(rx, ry)
                # 否则不合并（理论上不会发生，因为都在同一个 allowed_set 内）

    # 3) 常规“按共现从大到小”合并，但受锁定与 ≤3 限制
    def find(x):
        while parent[x] != x:
            parent[x] = parent[parent[x]]
            x = parent[x]
    ...
    for (a, b), _w in co.most_common():
        if a not in idx_of_label or b not in idx_of_label:
            continue
        try_union(idx_of_label[a], idx_of_label[b])
...
def _sample_nodes_on_polyline(poly: List[Point], grid_px: float) -> List[Point]:
    ...
    for (x0, y0), (x1, y1) in zip(poly, poly[1:]):
        if x0 == x1:
            ...
            x = min(x0, x1)
            x_end = max(x0, y1 if False else x1)  # 保持风格
            k = 1
...
def build_steiner_trunk_paths(
    panel_coord: Point,
    jb_xy: List[Point],
    jb_types: List[Set[str]],  # 每个 JB 的类型集合
    comp: List[int],
    *,
    walls: List[List[Point]],
    grid_px: float,
    ...
    """
    在一个分量里用“最短路生长”生成主干路径集合，并确保任何路径不引入 >3 种类型。
    返回：
      paths: 由若干折线段组成（每个新增 JB 的接入路径）
      joins: [(u_idx, v_idx, join_point, types_on_path)]
            u_idx: 0=Panel，>0=JB+1，-1 表示接入在树中间
            v_idx: 新 JB 的 index+1
            types_on_path: 此段路径的类型集合（≤3）
    """
    panel_grid = snap(panel_coord, grid_px)

    # 树节点集合 & 每个树节点的类型集合（沿主干已传播的集合）
    tree_nodes: List[Point] = [panel_grid]
    node_types: Dict[Point, Set[str]] = {panel_grid: set()}  # Panel 视为空集合

    paths: List[List[Point]] = []
    joins: List[Tuple[int, int, Point, Set[str]]] = []
    remaining: Set[int] = set(comp)

    # 选离 Panel 最近的 JB 作为种子
    ...
    # 此段类型 = Panel(∅) ∪ JB_types[first]
    types0 = set(jb_types[first])
    # 采样点加入树，并赋类型（整段沿途类型一致：types0）
    for p in _sample_nodes_on_polyline(path0, grid_px):
        if p not in node_types:
            node_types[p] = set(types0)
            tree_nodes.append(p)
        else:
            node_types[p] |= types0
            if len(node_types[p]) > 3:
                node_types[p] = set(list(node_types[p])[:3])  # 保守裁切，理论上不会触发
    paths.append(path0)
    joins.append((0, first + 1, path0[-1], set(types0)))
    ...
    while remaining:
        ...
        for j in list(remaining):
            ...
            Sj = set(jb_types[j])
            for t in cand_nodes:
                # 连接到 t 后，此段类型集合 = node_types[t] ∪ Sj
                U = set(node_types.get(t, set())) | Sj
                if len(U) > 3:
                    continue  # 逻辑上禁止超过 3 种
        ...
        if best_path is None:
            # 没有任何符合 ≤3 的树节点，兜底：直接连 Panel（Panel 类型 ∅ ∪ Sj ≤3 必然成立）
            j = remaining.pop()
            Sj = set(jb_types[j])
            path = route_rect_with_walls(
                panel_coord,
            ...
            paths.append(path)
            joins.append((0, j + 1, path[-1], set(Sj)))
            continue

        # 成功找到：把 best_j 接入到 best_to
        ...
        # 找 best_to 的“端点编号”：若恰为某 JB 中心则为该 JB，否则视为“树中间点”（-1）
        attach_idx = -1
        if best_to == panel_grid:
            attach_idx = 0
        else:
            for j0 in comp:
                if best_to == snap(jb_xy[j0], grid_px):
                    attach_idx = j0 + 1
                    break

        paths.append(best_path)
        joins.append((attach_idx, best_j + 1, best_path[-1], set(best_types)))
    ...
    *,
    ...
    for local_i, (x, y) in enumerate(kept_centres):
        ...
        draw_polyline_annot(
            ...
            width=wire_width,
            stroke_color=col_i,
            title=BRANCH_SUBJECT,  # 固定 (3) #10 IN 3/4" EMT
    ...
    for comp in comps:
        ...
        for poly, jinfo in zip(trunk_paths, joins):
            ...
            # subject：根据此段真实类型数（≤3）
            subj = mainbus_subject_for_type_count(len(types_on_path))
            L_pts = rect_path_length(poly)
            L_str = format_length_ft_in(L_pts, inch_precision=0)
    ...
    return {
        ...
        "trunk_edges": len(drawn_segments),
    }
