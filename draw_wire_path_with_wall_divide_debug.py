...
def build_steiner_trunk_paths(
    ...
    *,
    ...
    for j in comp:
        ...
        if start not in adj:
            if adj:
                ...
                print("[DEBUG LEFT BFS] adj is empty for comp", comp)
                continue
        node_path = bfs_path(start, panel_node)
        if not node_path or len(node_path) < 2:
            print(
                "[DEBUG LEFT BFS] fail JB",
                j,
                "start",
                start,
                "-> panel_node",
    ...
    print("[DEBUG steiner] panel", panel_coord, "comp", comp, "#paths", len(paths))
    return paths, joins
...
def _draw_side(
    ...
    *,
    ...
    if jb_prefix == "JBL":
        print("[DEBUG LEFT] num_jb:", len(jb_xy), "comps:", comps)
