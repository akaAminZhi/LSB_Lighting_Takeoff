...
def choose_lane_for_poly(
    poly: List[Point], lane_map: Dict[Tuple[Point, Point], Set[int]]
) -> int:
    used: Set[int] = set()
    for seg in _poly_to_base_segs(poly):
        used |= lane_map[seg]
    ...
    *,
    ...
    返回：
      ...
      joins: [(u_idx, v_idx, join_point, types_on_path)]
         u_idx: 0=Panel, >0=JB+1, -1=接入在树中间点
...
def _draw_side(
    ...
    *,
    ...
    for local_i, (x, y) in enumerate(kept_centres):
        ...
        # 并排：分配车道并平移；长度按未平移 poly
        lane = choose_lane_for_poly(poly, lane_map)
        poly_o = offset_poly_translate(poly, lane, lane_gap)
    ...
    for comp in comps:
        ...
        # 用“增长顺序”维护已连接锚点（0=Panel，其余=JB+1）
        ...
        for poly, jinfo in zip(trunk_paths, joins):
            ...
            if mark_virtual_steiner and u_idx < 0:
                ...
                if not connected_anchors:
                    merged_label = f"merge of {panel_name()} & {v_name}"
                else:
                    cand = connected_anchors + [v_idx]
                    # 求两两组合里到 join_point 的距离和最小的那对
                    best_pair = None
                    best_cost = None
                    ...
                    if best_pair:
                        ...
                        merged_label = f"merge near {v_name}"
    ...
    return {
        ...
        "trunk_edges": len(trunk_paths),  # 主干段数
    }
...
def route_with_jb_strategy_dual_panels(
    ...
    *,
    ...
    priority_families: Optional[List[Iterable[str]]] = None,
    on_line_tol_px: float = 2.0,
    lane_gap: float = 8.0,  # 并排间距：可调（建议≥线宽*2）
):
