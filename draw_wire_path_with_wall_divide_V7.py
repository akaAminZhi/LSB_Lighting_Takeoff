...
def _draw_side(
    ...
    *,
    ...
    allow_branch_overlap: bool,  # True=支线可重叠
    ...
    for local_i, (x, y) in enumerate(kept_centres):
        ...
        lane = choose_lane_for_poly(
            poly, lane_map, grid_px, allow_overlap=allow_branch_overlap
        )
    ...
    for comp in comps:
        ...
        for poly, jinfo in zip(trunk_paths, joins):
            ...
            # 主干是否允许重叠（默认 False）
            lane = choose_lane_for_poly(
                poly, lane_map, grid_px, allow_overlap=allow_trunk_overlap
            )
