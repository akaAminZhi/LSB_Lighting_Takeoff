...
def auto_place_junction_boxes(
    ...
    *,
    ...
    """
    关键修复：并排合并改为“**按标签分组**后再 DBSCAN”，确保原子设备点是**单标签**。
...
def pretty_node_name(idx: int) -> str:
    return "PANEL" if idx == 0 else f"JB{idx-1}"
    ...
    *,
    ...
    for j, pt in enumerate(jb_xy):
        draw_circle_annot(
            ...
            width=1.1,
            title="JB",
            contents=f"JB{j} | types:{sorted(list(jb_label_sets[j]))}",
        )
    ...
    for u, v in trunk_edges:
        ...
        name_u = pretty_node_name(u)
        name_v = pretty_node_name(v)
    ...
    # 面板
    draw_circle_annot(
        page, panel_coord, r=panel_radius, fill_color=color, width=0.4, title="PANEL"
    )
...
if __name__ == "__main__":
    ...
    PANEL_COORD: Point = (1941.0, 2416.0)
    ...
    WALLS = [
        ...
        [
            (1896.781982421875, 1370.4129638671875),
            (1963.10400390625, 1370.4129638671875),
            (1963.10400390625, 1272.5770263671875),
            (1848.3349609375, 1272.5770263671875),
            (1848.3349609375, 1371.35302734375),
        ],
        [
            (1813.0579833984375, 1369.9420166015625),
            (1769.7850341796875, 1369.9420166015625),
            (1769.7850341796875, 1273.989013671875),
            (1848.8060302734375, 1273.989013671875),
        ],
        [
    ...
    # 不使用 PRIORITY_FAMILIES 也能保证“同一标签只能在一个家族里”
    PRIORITY_FAMILIES = None  # 例如： [{"2","4","6"}]
    ...
    print("Route with JB (label-wise-merge + disjoint families):", info)
