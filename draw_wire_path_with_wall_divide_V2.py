...
def nudge_point_off_walls(
    ...
    *,
    ...
    """
    以 p 为中心在网格点上做螺旋搜索，找到最近“远离墙 >= clearance”的点。
    """
    if (
        not walls
        or clearance_px <= 0.0
...
def a_star_rectilinear(
    ...
    *,
    ...
    node_limit: int = 20000,  # 最大展开节点数
    deadline_s: float = 0.25,  # 单次 A* 最长运行时间（秒）
) -> Optional[List[Point]]:
    if grid_px <= 0:
        grid_px = 50.0
...
def route_rect_with_walls(
    ...
    *,
    ...
    max_expand_rounds: int = 1,  # 6→12
    max_refine_rounds: int = 1,  # g→g/2
    node_limit: int = 15000,
    deadline_s: float = 0.20,
) -> List[Point]:
    """
    先试 L 形；若撞墙，A* 搜索。A* 可逐步放大窗口/细化网格，且有节点上限与超时硬停。
    """
    # 先试 L 形（更干净）
    ...
    for expand_i in range(max_expand_rounds + 1):
        ...
        for _ in range(max_refine_rounds + 1):
            ...
            if path:
                return path
            cur_grid = max(cur_grid / 2.0, 5.0)  # 防止过小导致节点爆炸
...
def auto_place_junction_boxes(
    ...
    *,
    ...
    priority_families: Optional[List[Iterable[str]]] = None,
    walls: Optional[List[List[Point]]] = None,  # 新增：用于 JB 落点避墙
    clearance_px: float = 0.0,  # 新增：用于 JB 落点避墙
):
    """
    - 同标签并排合并（DBSCAN）得到“原子设备点”（单标签）
    - JB 初始 = 原子点（1:1），随后家族内贪心凝聚，标签并集 ≤ 3、容量 ≤ capacity、距离 ≤ eps_px
    - JB 中心吸附网格 + 避墙微调
    """
...
def jb_forest_with_per_component_panel_links(
    ...
    *,
    ...
    """
    仅允许同家族的 JB 分量相连（并集 ≤ max_types）；每个连通分量各自 homerun 到 Panel。
    ...
    *,
    ...
    for u, v in trunk_edges:
        ...
        draw_polyline_annot(
            ...
            stroke_color=color,
            title="main bus",
            contents=f"{node_with_types(u)} -> {node_with_types(v)} | L={L_str}",
        )
